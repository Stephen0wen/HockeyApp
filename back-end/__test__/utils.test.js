...
describe("Tests for toNestedArr", () => {
  ...
  test("Should the correct single nested array when passed an array containing a single object", () => {
    ...
    const expected = [
      ["Dibbert Inc", "Aaliyah", "Implemented motivating customer loyalty"],
    ];
    const actual = toNestedArr(input);
    expect(actual).toEqual(expected);
  });
...
describe("Tests for replaceProperty", () => {
  ...
  test("Should return a new array", () => {
    ...
    const actual = replaceProperty(input, "shop_name", "shop_id", testLookup);
    expect(actual).not.toBe(input);
  });
...
describe("generateLeagueTable", () => {
  it("should generate correct team stats for a single match", () => {
    ...
    const teamStats = generateLeagueTable(completedMatches);
